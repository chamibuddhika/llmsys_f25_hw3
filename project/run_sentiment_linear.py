...
def cross_entropy_loss(out, y):
    ...
    # ones = minitorch.tensor_functions.ones(y.shape, BACKEND)
    # out_softmax = minitorch.nn.logsoftmax(out, 0)
    # one_minus_out_softmax = minitorch.nn.logsoftmax(ones - out, 0) 
    #  
    # bce = y * out_softmax + (ones - y) * one_minus_out_softmax
    # return bce.mean()
...
class Linear(minitorch.Module):
    def __init__(self, in_size, out_size):
        ...
        self.weights = minitorch.Parameter(minitorch.rand((in_size, out_size), backend=BACKEND, requires_grad=True))
        self.bias = minitorch.Parameter(minitorch.rand((out_size,), backend=BACKEND, requires_grad=True)) 
    ...
    def forward(self, x):
        ...
        batch = x.shape[0]
        input = x.view(batch, self.in_size)
        
        mul = input @ self.weights.value
        mul = mul.view(batch, self.out_size)
        out = mul + self.bias.value
        return out
...
class Network(minitorch.Module):
    ...
    def forward(self, embeddings):
        ...
        linear1_out = self.linear1(embeddings) 

        if self.training:
            linear2_in = minitorch.nn.dropout(linear1_out.relu(), self.dropout_prob)
        else:
            linear2_in = linear1_out.relu()
        linear2_out = self.linear2(linear2_in)
        out = linear2_out.sigmoid().view(embeddings.shape[0])
        return out
    ...
    def train(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for batch_num, example_num in enumerate(
                ...
                x = minitorch.tensor(X_train[example_num : example_num + batch_size], BACKEND, requires_grad=True)
                y = minitorch.tensor(y_train[example_num : example_num + batch_size], BACKEND, requires_grad=True)
