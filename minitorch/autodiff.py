...
def topological_sort(variable: Variable) -> Iterable[Variable]:
    ...
    def _dfs(node, visited, result):
        if not node.is_constant():
            visited.add(node.unique_id)
            for parent in node.parents:
              if parent.unique_id not in visited:
                _dfs(parent, visited, result)
            result.append(node)
        
    _dfs(variable, visited, result)
    return result[::-1]
    # END ASSIGN1_1
...
def backpropagate(variable: Variable, deriv: Any) -> None:
    ...
    for node in nodes:
        d_out = grads.get(node.unique_id, 0)
        if node.is_leaf():
            ...
            for parent, d_parent in node.chain_rule(d_out):
                if d_parent is None:
                    continue
                grads[parent.unique_id] = grads[parent.unique_id] + d_parent
