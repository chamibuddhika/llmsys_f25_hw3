...
__global__ void MatrixMultiplyKernel(
   ...
   */
    ...
    const int a_batch_stride = a_shape[0] > 1 ? a_strides[0] : 0;
    const int b_batch_stride = b_shape[0] > 1 ? b_strides[0] : 0;
    const int out_batch_stride = out_shape[0] > 1 ? out_strides[0] : 0;

    const int m = out_shape[1];      // == a_shape[1]
    const int p = out_shape[2];      // == b_shape[2]
    const int n = a_shape[2];        // == b_shape[1]
    
    // const int N = blockDim.y * gridDim.y;
    const int n_tiles = (n + TILE - 1) / TILE;
    
    float cval = 0;
    for (int i = 0; i < n_tiles; i++) {
      const int a_c = i * TILE + threadIdx.y;  // 0..n-1
      const int b_r = i * TILE + threadIdx.x;  // 0..n-1

      if (row < m && a_c < n) {
        a_shared[threadIdx.x][threadIdx.y] = a_storage[a_batch_stride * batch + row * a_strides[1] + a_c * a_strides[2]];
      } else {
        a_shared[threadIdx.x][threadIdx.y] = 0;
      }

      if (col < p && b_r < n) {
        b_shared[threadIdx.x][threadIdx.y] = b_storage[b_batch_stride * batch + b_r * b_strides[1] + col * b_strides[2]];
      } else {
      ...
      for (int k = 0; k < TILE; k++) {
        cval += a_shared[threadIdx.x][k] * b_shared[k][threadIdx.y];
      }
      __syncthreads();
    }

    if (row < m && col < p) {
      out[out_batch_stride * batch + row * out_strides[1] + col * out_strides[2]] = cval;
    }
...
__global__ void mapKernel(
   ...
   */
    ...
    // Convert linear position -> multidim out_index
    to_index(tid, out_shape, out_index, shape_size);

    // Broadcast out_index -> in_index according to in_shape
    broadcast_index(out_index, out_shape, in_shape, in_index, shape_size, shape_size);

    // Compute positions via strides
    ...
    // Apply unary fn and write to out.
